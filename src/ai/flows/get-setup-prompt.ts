...
const setupTaskInstructions: Record<SetupStepType, string> = {
  ...
  [SetupStepEnum.enum.ASK_APPEARANCE_DESCRIPTION]: "The user, {{userName}}, just told you their topic preferences in their last message: '{{userRawInput}}'. Now, ask them to describe how they'd like you to look. Give a brief example like 'e.g., long curly red hair, green eyes, and a friendly smile'. Explain that this description will help generate visual options for them to choose from.",
...
const getSetupPromptFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    // For INITIATE_SETUP, userRawInput is a language hint.
    // For other steps, it's the actual user's previous message.
...
Your current task: ${taskInstruction}`;
    // Note: taskInstruction itself can contain {{userName}} and {{userRawInput}} which will be substituted by Handlebars.
